#!/usr/bin/env node
/**
 * The COMPOSED admission capture.
 *
 * Same harness as pi-governance's capture/capture.mjs, re-pointed at the composed stack:
 *
 *   capture host (ACP client)
 *     ⇄ stdio ⇄ pi-acp@0.0.32 (the carrier crew resolves)            node_modules/pi-acp/dist/index.js
 *       ── PI_ACP_PI_COMMAND ─▶ wicked-crew's wicked-pi.mjs shim      (copied verbatim, unmodified)
 *         ── WICKED_PI_BINARY ─▶ gate-pi.mjs                          (prepends the gate flags)
 *           ──▶ real pi 0.84.2, `-ne -e <pi-governance extension.js>`
 *
 * pi-governance's own ~540-line adapter (dist/cli.js, src/acp-agent.ts, src/pi-rpc.ts) is NOT in
 * this path. The only pi-governance code loaded is src/extension.ts (compiled: dist/extension.js).
 *
 ...
 *
 *   node capture-composed.mjs [--out <dir>] [--only <scenario>]
 */
...
const PIGOV = resolve(HERE, "..", "pigov", "pi-governance");
const CARRIER = join(HERE, "node_modules", "pi-acp", "dist", "index.js");
const SHIM = join(HERE, "bridges", "wicked-pi.mjs");
const GATE_PI = join(HERE, "gate-pi.mjs");
const EXTENSION = join(PIGOV, "dist", "extension.js");
const SEQ_PROVIDER = join(PIGOV, "capture", "scripted-provider.ts");
...
async function runScenario(scenario, outDir) {
	...
	// injected by gate-pi.mjs so the shim itself stays byte-identical to crew's copy.
...
function versions() {
	...
	const carrier = JSON.parse(readFileSync(join(HERE, "node_modules", "pi-acp", "package.json"), "utf8"));
	let pigovCommit;
	try {
		pigovCommit = execFileSync("git", ["rev-parse", "HEAD"], { cwd: PIGOV, encoding: "utf8" }).trim();
	} catch {
...
async function main() {
	...
	const outDir = outIndex >= 0 ? resolve(process.argv[outIndex + 1]) : join(HERE, "evidence");
	...
	for (const path of [CARRIER, SHIM, GATE_PI, EXTENSION, SEQ_PROVIDER, PAR_PROVIDER]) {
		if (!existsSync(path)) fail(`missing composed-path component: ${path}`);
	}
	const meta = versions();
